...
SOLID
	...
	O - open-close  -   otwarty-zamkniêty, otwarty na rozsze¿enia a zamkniêty na modyfikacje
*/
...
int main()
{
	...
	while (true)
	{
		...
		{
			drawBoard(board, ROW_COUNT, COL_COUNT);
			std::cout << "Wygrana!!\n Wygra³ " << playersSymbol[currentPlayer] << "\n";
			break;
		}
		...
		//zmienia gracza na nastêpnego
		currentPlayer = (PLAYERS_COUNT + currentPlayer + 1) % PLAYERS_COUNT;
		//jesli chemy iœæ w dróg¹ stronê 3-2-1-0-3-2...
		//currentPlayer = (PLAYERS_COUNT + currentPlayer - 1) % PLAYERS_COUNT;
